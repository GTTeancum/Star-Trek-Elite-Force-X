...
try:
 with a.output.open('x') as out, a.flight.open() as stream:
  ...
  while time.monotonic()-start<a.seconds:
   ...
   counts={}
   for suffix in ('SplitP2SceneConsidered','SplitP2SceneAdded','SplitP2SceneSelfAdded'):
    raw=g.virtual(va('_g_SPXB'+suffix),4);counts[suffix]=struct.unpack('<I',raw)[0] if len(raw)==4 else None
   record={'scene':counts,'epoch':time.time(),'flight_t_latest':latest,'players':players,'camera':bool(camera[0]) if len(camera)==1 else None,'physical_free':struct.unpack('<I',free)[0] if len(free)==4 else None,'atomic':False}
